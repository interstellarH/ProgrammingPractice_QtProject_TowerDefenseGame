...
void MainWindow::paintEvent(QPaintEvent* event)
{
    ...
    foreach(const Defend_Tower* defendtower,m_defendtowerList)
        defendtower->draw(&painter);
}
...
void MainWindow::mousePressEvent(QMouseEvent *qm)//鼠标点击函数
{
    QPoint nowpo=qm->pos();
    auto begin=m_towerPositionList.begin();
    auto end=m_towerPositionList.end();
    while(begin!=end)
    {
        if(qm->button()==Qt::LeftButton)
        {
            if(begin->ContainPos(nowpo))
            {
                if(!begin->hasTower())
                {
                    begin->sethasTower(1);
                    Defend_Tower* newtower=new Defend_Tower(begin->getCenterPos(),this,1);
                    m_defendtowerList.push_back(newtower);
                    update();
                    break;
                }
            }
