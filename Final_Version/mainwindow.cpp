...
void MainWindow::awardGold()//每次杀死敌人后的金币奖励
...
{
    painter->save();
    painter->setPen(QPen(Qt::red,3));//这里具体的参数确定了位置
...
void MainWindow::drawHp(QPainter *painter) const
{
    ...
    else painter->drawPixmap(170,0,50,50,QPixmap(":/new/prefix1/resource1/JiaYing.png"));
    painter->restore();
}
...
bool MainWindow::loadWaves()
{
    ...
    //int enemyStartInterval1[]={11000,12000,14000,16000,8000,10000,13000,15000,18000,20000};//第一波战线比较长，留出时间建塔
    ...
    for(int i=0;i<10;++i)
    {
       ...
       // Enemy * enemy=new Enemy(startWayPoint,this,3);//加入类型参数
        m_enemyList.push_back(enemy);
        enemy->reSetHp(enemy->e_maxHp+50*m_waves);//波数增加，怪物的血量增加，一次加5点
        ...
        //QTimer::singleShot(enemyStartInterval2[i],enemy,SLOT(doActive()));
    }
    return true;
}
