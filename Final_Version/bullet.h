...
{
    Q_OBJECT
    //自定义属性名，这里一开始写的是m_currentPos，但是程序里用的是b_currentPos，所以找不见，会报错
    //you’re trying to animate a non-existing property windsowOpacity of your QObject
    //这也是为什么我们看不见子弹
    Q_PROPERTY(QPoint b_currentPos READ getCurrentPos WRITE setCurrentPos)
public:
    Bullet();
...
protected:
    ...
    int b_damage;//物理攻击值，与防御塔一致,应该保持在敌人血量的
                     //保证每秒对单个敌人的攻击达到普通敌人血线的50% 预设值在10-50之间
    double b_magical;//魔法攻击值，与防御塔一致
                      //保证每秒对单个敌人的攻击达到普通敌人血线的50% 预设值在10-50之间
