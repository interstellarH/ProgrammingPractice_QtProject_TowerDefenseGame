...
void Enemy::draw(QPainter *painter) const
{
    ...
    if(!e_game->getJiaYingMod()) painter->drawPixmap(temp.x(),temp.y(),e_sprite);
        ...
        if(e_type==1||e_type==3){
            sprite=QPixmap(":/new/prefix1/resource1/CM.png");
        }
        else if(e_type==2)sprite=QPixmap(":/new/prefix1/resource1/CM2.png");
        painter->drawPixmap(temp.x(),temp.y(),40,40,sprite);
    }
...
{
    if(!e_active) return;
    if(collisionWithCircle(e_pos,2,e_destinationWayPoint->getPos(),3))//arriving at WayPoint
    {
...
void Enemy::getFrozen()//减速塔效果仅对最快的那种怪兽有用
{
    if(e_speed>1) e_speed=1;
}
void Enemy::removeFrozen()//解除被定住负面状态
{
    e_speed=1;//试一下看看能不能看到恢复
}
