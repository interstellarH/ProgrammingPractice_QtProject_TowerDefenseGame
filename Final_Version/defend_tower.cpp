...
void Defend_Tower::shootWeapon()
{
    ...
    if(t_type==1){
        ...
        b=new Frozen_Bullet(t_pos,t_attacker->getPos(),this,this->t_game,100);
    }
    b->move();
    t_game->addBullet(b);
}
...
void Defend_Tower::draw(QPainter* painter)const
{
    painter->save();
    if(t_type==2){//魔法环特效
        painter->drawPixmap(t_pos.x()-50,t_pos.y()-50,QPixmap(":/new/prefix1/resource1/Magical_effect.png"));
    }
    painter->setPen(QPen(Qt::red,2,Qt::DashDotLine));//可以采用更漂亮的颜色哦
...
void Defend_Tower::reSetDamage(int damage){
    t_damage=damage;
}
